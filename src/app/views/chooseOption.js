            ...
            response.data.lots.map(lot => {      
                lot.tasks.forEach((task,index) => {
                    ...
                    if(moment(lot.createdDate).add(task.days, 'minutes').format() <= moment().format() && !task.state){
                        ...
                        const t =  Axios('/api/pushNotifications/newNotification',{method:'POST',data: {message:task.name,imageNotification:task.imageURL, url: lot._id}}).then(res => console.log(res) ).catch(err => console.log(err))
                    }
                })
            })
        }catch(error){
